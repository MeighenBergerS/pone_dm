...
class Background(object):
        ...
        if self.name == "IceCube":
            ...
            if config['general']['pone type'] == 'old':
                ...
                pickle.dump(self._bkgrd, open(
                 '../data/tmp_files/Christian_%f/Back_christ.pkl' % (_spacing),
                 'wb'))
                pickle.dump(counts_an, open(
                 '../data/tmp_files/Christian_%f/' +
                 'Back_christ_ang_.pkl' % (_spacing),
                 'wb'))
                # _log.info("Trying to load pre-calculated tables")
                # _log.debug("Searching for Atmospheric and Astro Fluxes")
    ...
    def bkgrd(self):
        """Returns backgorund counts dict [ label : Neutrino Flavour ]
        """
        odr_model = Model(self.odr_fit_phi)
        e_bins = self.width2grid(self._shower._egrid)
        odr_data = RealData(e_bins[36:71],
                            self.bkgrd_data[36:71])
        odr_fit = ODR(odr_data, odr_model, beta0=[2.00735973e+00,
                                                  2.91299035e+03, 1])
        odr_fit.set_job(fit_type=0)
        odr_out = odr_fit.run()
        odr_out.beta
        fit_phi_odr = self.odr_fit_phi(odr_out.beta, self._shower._egrid)
        return fit_phi_odr
