...
class Limits(object):
    ...
    shower_sim: Atm_Shower
        A Atm_Shower object
    """
    def __init__(self, aeff: Aeff, dm_nu: DM2Nu, shower_sim: Atm_Shower, detector: Detector):
...
# Limit calculation for IceCube------------------
    ...
    def limit_calc_ice(self):
        self._signal_flux = {}
        self._signal_counts = {}
        bkgrd = self._detector(self._aeff, self._dmnu, self._shower).sim2dec()
        y = {}
...
# Limit calculation for Pone----------------------
    ...
    def limit_calc_pone(self,
        ...
        for i in (config['atmospheric showers']['particles of interest']):
            ...
            self._t_d = self._find_nearest(self._egrid, 5e2)
            self._limit_scan_grid_base = np.array([[
                                                    (self._signal_calc_pone(self._egrid, mass,
                                                     sv, config['atmospheric showers']['theta angles']
                                                    )**2.)[self._t_d:] /
                                                    bkgrd[i][self._t_d:]
            ...
            y[i] = np.array([[chi2.sf(np.sum(np.nan_to_num(x)), 2)  # TODO: chi2 needs to be defined or imported
                            for x in k] for k in self._limit_scan_grid_base
                            ])
        return y
